...
(fset 'jco/cpp-fix-constr-destr
      [?V ?j ?= ?\M-x ?j ?c ?o ?/ ?c ?p ?p ?- ?i ?n ?s ?e ?r ?t ?- ?c ?l ?a ?s
          ?s ?- ?n ?a ?m ?e return ?i ?: ?: escape ?f ?\( ?l ?\" ?t ?y ?i ?w ?f
          ?= ?h ?d ?t ?\) ?l ?s return ?\{ backspace ?: ? ?\C-r ?t ?\( ?p ?a ?r
          ?e ?n ?t escape ?o ?\{ return escape ?d ?d ?o escape ?j ?d ?w ?\M-x ?j
          ?c ?o ?/ ?c ?p ?p ?- ?i ?n ?s ?e ?r ?t ?- ?c ?l ?a ?s ?s ?- ?n ?a ?m
          ?e return ?i ?: ?: escape ?f ?\) ?a ? ?= ? ?d ?e ?f ?a ?u ?l ?t escape
          ?j])

(fset 'jco/cpp-def-to-decl
      [?0 ?2 ?f ?: ?l ?d ?B ?/ ?\) return ?A ?\; escape ?V ?\C-o ?= ?\C-x ?\ ])

(fset 'jco/cpp-variadic-templatize
      [?0 ?O ?t ?e ?m ?p ?l ?a ?t ?e ?< ?t ?y ?p ?e ?n ?a ?m ?e ?. ?. ?. ?\S- ?T
          ?s ?> escape ?0 ?j ?f ?\( ?l ?c ?i ?\) ?T ?s ?& ?& ?. ?. ?. ? ?a ?r ?g
          ?s escape ?0 ?j])

(fset 'jco/cpp-make-const-ref
      [?i ?c ?o ?n ?s ?t ?  escape ?E ?a ?& escape])
  ...
  (save-excursion
    (save-restriction
      ...
      (save-excursion
        (while
            (re-search-forward
             "\\(static \\|virtual \\|Q_INVOKABLE \\| override\\|explicit \\)"
             jco/end-of-decl t)
         (replace-match "")))
