...
(defun jco/kakoune ()
  "Open current buffer in Kakoune."
  (interactive)
  (when (display-graphic-p)
    (start-process "kakoune" nil
                   "alacritty"
                   "-e"
                   "tmux"
                   "new"
...
(general-def 'insert "C-M-SPC" 'hippie-expand)
...
(use-package smartparens
  ...
  (sp-with-modes (cl-set-difference (cl-union sp-lisp-modes '(lisp-data-mode))
                                    sp-clojure-modes)
...
(set-frame-font "FiraCodeNerdFont 11" nil t)
...
;; Display all errors when starting `flyspell-mode'.
...
(use-package org-noter
  :defer 1
  :straight (org-noter :host github :repo "joncol/org-noter" :branch "add-focus-behavior-setting")
  :if (and (not (eq system-type 'windows-nt))
           (display-graphic-p))
  :bind (:map org-noter-doc-mode-map
              (("M-I" . org-noter-insert-note)))
  :custom
  (org-noter-focus-doc-after-note-navigation nil)
  :config
...
(defun configure-ef-themes ()
  ...
  (unless (ef-themes--list-enabled-themes)
